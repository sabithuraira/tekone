<?php

use yii\helpers\Html;
use yii\widgets\ActiveForm;

/* @var $this yii\web\View */
/* @var $model app\models\Aramsearch */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="aram-search">

    <?php $form = ActiveForm::begin([
        'action' => ['index'],
        'method' => 'get',
    ]); ?>

    <?= $form->field($model, 'id') ?>

    <?= $form->field($model, 'padisawah') ?>

    <?= $form->field($model, 'padiladang') ?>

    <?= $form->field($model, 'padi') ?>

    <?= $form->field($model, 'jagung') ?>

    <?php // echo $form->field($model, 'kedelai') ?>

    <?php // echo $form->field($model, 'kacangtanah') ?>

    <?php // echo $form->field($model, 'kacanghijau') ?>

    <?php // echo $form->field($model, 'ubikayu') ?>

    <?php // echo $form->field($model, 'ubijalar') ?>

    <?php // echo $form->field($model, 'fenomena') ?>

    <?php // echo $form->field($model, 'id_wil') ?>

    <?php // echo $form->field($model, 'id_satuan') ?>

    <?php // echo $form->field($model, 'id_tahun') ?>

    <?php // echo $form->field($model, 'created_at') ?>

    <?php // echo $form->field($model, 'updated_at') ?>

    <div class="form-group">
        <?= Html::submitButton('Search', ['class' => 'btn btn-primary']) ?>
        <?= Html::resetButton('Reset', ['class' => 'btn btn-default']) ?>
    </div>

    <?php ActiveForm::end(); ?>

</div>
